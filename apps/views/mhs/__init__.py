...
from flask.ext.peewee.utils import object_list
from peewee import JOIN, fn

from apps.forms.others import KumpulkanTugasForm
from apps.models import User, Tugas, KumpulTugas, MataKuliah, Post, Level, Phile, Profile
...
def home():
    dosens = (User.select(User, MataKuliah)
              ...
              .join(Tugas, JOIN.LEFT_OUTER, on=(Tugas.mata_kuliah == MataKuliah.id))
    ...
    return render_template('mhs/home.html', dosen=dosen, dosens=dosens, posts=posts, dosen_id=dosen_id)
...
def tugas_detail(tugas_id):
    ...
    return render_template('mhs/detail.html', tugas=tugas, dosens=dosens, form=form,
                           sudah_mengumpulkan=sudah_mengumpulkan, dosen_id=dosen_id)
